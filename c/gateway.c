...
char** createCharArray(int m, int n)
{
	...
	char** rows;
  	...
  	values = calloc(m*n, sizeof(float));
  	rows = malloc(n*sizeof(float*));
...
int getGateway(PIP_ADAPTER_INFO pAdapter,char **gateway){
	...
	count=0;
	while (pAdapter) {
		if(strcmp(pAdapter->GatewayList.IpAddress.String,t)!=0){
			strcpy(gateway[count],pAdapter->GatewayList.IpAddress.String);
			count++;
...
int printAdapterInfo(PIP_ADAPTER_INFO pAdapter){
  UINT i;
  while (pAdapter) {
		printComboIndex(pAdapter);
		printAdapterName(pAdapter);
		printDescription(pAdapter);
		printAdapterAddr(pAdapter);
		printIndex(pAdapter);
		printIndex(pAdapter);
		printType(pAdapter);
		printIPAddress(pAdapter);
		printIPMask(pAdapter);
		printGateway(pAdapter);
		printWins(pAdapter);
	/////////////////////////////////////
  	pAdapter = pAdapter->Next;
    printf("\n");
